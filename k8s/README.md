...
=== Get Nodes
```
kubectl --kubeconfig ~/.kube/opensbpm@hetzner.yaml get nodes
```

=== Label Nodes
```
kubectl --kubeconfig ~/.kube/opensbpm@hetzner.yaml label node workernode-1 service=thirdparty
kubectl --kubeconfig ~/.kube/opensbpm@hetzner.yaml label node workernode-2 service=engine
kubectl --kubeconfig ~/.kube/opensbpm@hetzner.yaml label node controlplane service=engine
```
